...
local Breathe
-- Forward-declare SafeCall supaya BAGIAN A (UI) bisa pakai sebelum definisi asli di BAGIAN 4.
local SafeCall
do
...
-- ===== BAGIAN A : SELF-CONTAINED UI (NO HTTP LOADSTRING) =====
-- UI custom supaya SELALU muncul. Implementasi API yang sama dengan adapter
-- Orion lama: Window:AddTab / wrapTab (AddSection,AddParagraph,AddButton,
-- AddToggle,AddDropdown,AddSlider,AddInput,AddColorPicker) + :OnChanged,
-- Fluent:Notify, CreateStatus, Window:SelectTab, Window:Destroy.

local UI_GUI_PARENT = (gethui and gethui()) or game:GetService("CoreGui")
if not UI_GUI_PARENT then UI_GUI_PARENT = LocalPlayer:WaitForChild("PlayerGui") end
pcall(function()
    local old = UI_GUI_PARENT:FindFirstChild("NoMercyHubUI")
    if old then old:Destroy() end
end)

local ScreenGui = Instance.new("ScreenGui")
ScreenGui.Name = "NoMercyHubUI"
ScreenGui.ResetOnSpawn = false
ScreenGui.IgnoreGuiInset = true
ScreenGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
SafeCall(function() ScreenGui.Parent = UI_GUI_PARENT end)
if not ScreenGui.Parent then ScreenGui.Parent = LocalPlayer:WaitForChild("PlayerGui") end

-- Warna default (Matrix Green) — akan di-update oleh SetTheme via theme listener
local UIColors = {
    Bg       = Color3.fromRGB(6, 14, 10),
    Dark     = Color3.fromRGB(10, 22, 16),
    Panel    = Color3.fromRGB(14, 30, 22),
    Accent   = Color3.fromRGB(0, 255, 102),
    Green    = Color3.fromRGB(46, 204, 113),
    Red      = Color3.fromRGB(231, 76, 60),
    White    = Color3.fromRGB(235, 245, 240),
    TextDim  = Color3.fromRGB(120, 160, 140),
    Border   = Color3.fromRGB(0, 255, 102),
}

-- Window utama
local MainWindow = Instance.new("Frame", ScreenGui)
MainWindow.Name = "MainWindow"
MainWindow.Size = UDim2.new(0, 560, 0, 400)
MainWindow.Position = UDim2.new(0.5, -280, 0.5, -200)
MainWindow.BackgroundColor3 = UIColors.Bg
MainWindow.BorderSizePixel = 0
MainWindow.Active = true
MainWindow.Draggable = true
do
    local c = Instance.new("UICorner", MainWindow); c.CornerRadius = UDim.new(0, 10)
    local s = Instance.new("UIStroke", MainWindow); s.Color = UIColors.Border; s.Thickness = 1.2; s.Transparency = 0.35
end
...
Header.BackgroundColor3 = UIColors.Dark
Header.BorderSizePixel = 0
do
    local c = Instance.new("UICorner", Header); c.CornerRadius = UDim.new(0, 10)
end
local Title = Instance.new("TextLabel", Header)
Title.Size = UDim2.new(1, -90, 1, 0)
Title.Position = UDim2.new(0, 14, 0, 0)
Title.BackgroundTransparency = 1
Title.Text = "NO MERCY — VIOLENCE DISTRICT  v4.1"
Title.TextColor3 = UIColors.White
...
MinBtn.BackgroundColor3 = UIColors.Panel
MinBtn.Text = "—"
MinBtn.TextColor3 = UIColors.White
MinBtn.Font = Enum.Font.GothamBold
MinBtn.TextSize = 14
do local c = Instance.new("UICorner", MinBtn); c.CornerRadius = UDim.new(0, 6) end

local CloseBtn = Instance.new("TextButton", Header)
CloseBtn.Size = UDim2.new(0, 28, 0, 28)
CloseBtn.Position = UDim2.new(1, -32, 0.5, -14)
CloseBtn.BackgroundColor3 = UIColors.Red
CloseBtn.Text = "X"
CloseBtn.TextColor3 = UIColors.White
...
Sidebar.Size = UDim2.new(0, 138, 1, -46)
Sidebar.Position = UDim2.new(0, 8, 0, 42)
Sidebar.BackgroundColor3 = UIColors.Dark
Sidebar.BorderSizePixel = 0
Sidebar.ScrollBarThickness = 2
Sidebar.AutomaticCanvasSize = Enum.AutomaticSize.Y
Sidebar.CanvasSize = UDim2.new(0, 0, 0, 0)
do
    local c = Instance.new("UICorner", Sidebar); c.CornerRadius = UDim.new(0, 8)
    local sideLayout = Instance.new("UIListLayout", Sidebar)
    sideLayout.Padding = UDim.new(0, 4)
    sideLayout.HorizontalAlignment = Enum.HorizontalAlignment.Center
end

-- Container (area konten tab)
local Container = Instance.new("Frame", MainWindow)
Container.Name = "Container"
Container.Size = UDim2.new(1, -156, 1, -46)
Container.Position = UDim2.new(0, 148, 0, 42)
Container.BackgroundColor3 = UIColors.Panel
Container.BorderSizePixel = 0
do local c = Instance.new("UICorner", Container); c.CornerRadius = UDim.new(0, 8) end

-- Kondisi minimize
local isMin = false
local function applyMinimize()
    if isMin then
        MainWindow.Size = UDim2.new(0, 560, 0, 38)
...
MinBtn.MouseButton1Click:Connect(function() isMin = not isMin; applyMinimize() end)
...
local function showTab(idx)
    for i, t in ipairs(_tabs) do
        ...
        t.Btn.BackgroundColor3 = (i == idx) and UIColors.Accent or UIColors.Dark
        t.Btn.TextColor3 = (i == idx) and UIColors.Bg or UIColors.TextDim
    end
end
...
local function makeTabButton(name, icon)
    ...
    btn.BackgroundColor3 = UIColors.Dark
    btn.Text = "  " .. name
    btn.TextColor3 = UIColors.TextDim
...
local function nextOrder() _orderCounter = _orderCounter + 1; return _orderCounter end

local function addCorner(parent, r)
    local c = Instance.new("UICorner"); c.CornerRadius = UDim.new(0, r or 6); c.Parent = parent; return c
end

local function wrapControl(callback)
    local proxy = {}
    proxy._callback = callback
    function proxy:OnChanged(cb) self._callback = cb; return self end
    return proxy
end

local function AddToggle(parent, title, default, onToggle)
    local holder = Instance.new("Frame", parent)
    holder.Size = UDim2.new(1, 0, 0, 34)
    holder.BackgroundColor3 = UIColors.Dark
    ...
    lbl.TextSize = 11
    lbl.TextColor3 = UIColors.White
    lbl.TextXAlignment = Enum.TextXAlignment.Left

    local dot = Instance.new("Frame", holder)
    dot.Size = UDim2.new(0, 32, 0, 16)
    dot.Position = UDim2.new(1, -40, 0.5, -8)
    dot.BackgroundColor3 = default and UIColors.Accent or Color3.fromRGB(50, 55, 60)
    addCorner(dot, 8)

    local knob = Instance.new("Frame", dot)
    knob.Size = UDim2.new(0, 12, 0, 12)
    knob.Position = default and UDim2.new(1, -14, 0.5, -6) or UDim2.new(0, 2, 0.5, -6)
    knob.BackgroundColor3 = UIColors.White
    addCorner(knob, 6)
    ...
    holder.InputBegan:Connect(function(inp)
        if inp.UserInputType == Enum.UserInputType.MouseButton1 or inp.UserInputType == Enum.UserInputType.Touch then
            state = not state
            dot.BackgroundColor3 = state and UIColors.Accent or Color3.fromRGB(50, 55, 60)
            knob:TweenPosition(
                state and UDim2.new(1, -14, 0.5, -6) or UDim2.new(0, 2, 0.5, -6),
                Enum.EasingDirection.Out, Enum.EasingStyle.Quart, 0.15, true)
...
local function AddSlider(parent, title, min, max, default, increment, onVal)
    local holder = Instance.new("Frame", parent)
    ...
    bar.BackgroundColor3 = Color3.fromRGB(40, 45, 50)
    addCorner(bar, 3)
    ...
    local function setFromX(x)
        ...
        if onVal then pcall(onVal, val) end
    end
...
local function AddDropdown(parent, title, options, defaultIdx, onPick)
    local holder = Instance.new("Frame", parent)
    holder.Size = UDim2.new(1, 0, 0, 34)
    holder.BackgroundColor3 = UIColors.Dark
    holder.BorderSizePixel = 0
    ...
    local curText = (options and options[defaultIdx or 1]) or ""
    local lbl = Instance.new("TextLabel", holder)
    lbl.Size = UDim2.new(1, -30, 1, 0)
    lbl.Position = UDim2.new(0, 10, 0, 0)
    lbl.BackgroundTransparency = 1
    lbl.Text = title .. ": " .. tostring(curText)
    lbl.Font = Enum.Font.GothamMedium
    lbl.TextSize = 11
    lbl.TextColor3 = UIColors.White
    lbl.TextXAlignment = Enum.TextXAlignment.Left
    ...
    arrow.Text = "v"
    arrow.TextColor3 = UIColors.TextDim
    ...
    list.BackgroundColor3 = UIColors.Panel
    list.BorderSizePixel = 0
    list.LayoutOrder = holder.LayoutOrder + 1
    list.Visible = false
    addCorner(list, 6)
    ...
    local function rebuild()
        ...
        for _, opt in ipairs(options or {}) do
            ...
            b.BackgroundColor3 = UIColors.Dark
            b.Text = "  " .. tostring(opt)
            b.TextColor3 = UIColors.White
            b.TextSize = 11
    ...
    holder.InputBegan:Connect(function(inp)
        if inp.UserInputType == Enum.UserInputType.MouseButton1 or inp.UserInputType == Enum.UserInputType.Touch then
            open = not open
            if open then
                list.Size = UDim2.new(1, 0, 0, math.min(#(options or {}), 6) * 26 + 4)
                list.Visible = true
            else
                list.Size = UDim2.new(1, 0, 0, 0)
                list.Visible = false
...
local function AddParagraph(parent, title, content)
    ...
    local c = Instance.new("TextLabel", holder)
    c.Size = UDim2.new(1, -16, 1, -22)
    c.Position = UDim2.new(0, 8, 0, 20)
    c.BackgroundTransparency = 1
    c.Text = content or ""
    c.Font = Enum.Font.Gotham
    c.TextSize = 10
    c.TextColor3 = UIColors.TextDim
    c.TextXAlignment = Enum.TextXAlignment.Left
    c.TextWrapped = true
    c.AutomaticSize = Enum.AutomaticSize.Y
    return {
        SetDesc = function(self, txt) c.Text = tostring(txt or "") end,
    }
end
...
local function AddSection(parent, title)
    ...
    lbl.TextSize = 11
    lbl.TextColor3 = UIColors.Accent
    lbl.TextXAlignment = Enum.TextXAlignment.Left
    return sep
end

local function AddInput(parent, title, default, onVal)
    local holder = Instance.new("Frame", parent)
    ...
    lbl.Font = Enum.Font.GothamMedium
    lbl.TextSize = 10
    lbl.TextColor3 = UIColors.TextDim
    lbl.TextXAlignment = Enum.TextXAlignment.Left
    ...
    box.Text = default or ""
    box.TextColor3 = UIColors.White
    box.Font = Enum.Font.Gotham
    box.TextSize = 11
    box.ClearTextOnFocus = false
    box.TextXAlignment = Enum.TextXAlignment.Left
    addCorner(box, 4)
    box.FocusLost:Connect(function() if onVal then pcall(onVal, box.Text) end end)
    return { _value = default or "" }
end

-- ---------- wrapTab: adapter API sama dengan Orion ----------
local function wrapTab(btn, page)
    local out = { _page = page }
    function out:AddSection(data)
        local name = type(data) == "table" and (data.Name or data.Title) or data
        return AddSection(page, tostring(name or "Menu"))
    end
    function out:AddParagraph(data) return AddParagraph(page, data.Title, data.Content) end
    function out:AddButton(data) return AddButton(page, data.Title, data.Callback) end
    function out:AddToggle(id, data)
        local proxy = wrapControl(nil)
        AddToggle(page, data.Title, data.Default, function(v) if proxy._callback then proxy._callback(v) end end)
        return proxy
    end
    function out:AddDropdown(id, data)
        local proxy = wrapControl(nil)
        AddDropdown(page, data.Title, data.Values or {}, data.Default, function(v) if proxy._callback then proxy._callback(v) end end)
        return proxy
    end
    function out:AddSlider(id, data)
        local proxy = wrapControl(nil)
        AddSlider(page, data.Title, data.Min or 0, data.Max or 100, data.Default or 0, data.Rounding or 1, function(v) if proxy._callback then proxy._callback(v) end end)
        return proxy
    end
    function out:AddInput(id, data)
        local proxy = wrapControl(nil)
        AddInput(page, data.Title, data.Default or "", function(v) if proxy._callback then proxy._callback(v) end end)
        return proxy
    end
    function out:AddColorPicker(data)
        return wrapControl(nil)
    end
    return out
end
...
function Window:AddTab(data)
    local name = data.Title or "Tab"
    local btn = makeTabButton(name, data.Icon)
    local page = makePage()
    local idx = #_tabs + 1
    local t = { Btn = btn, Page = page, Name = name }
    _tabs[idx] = t
    _tabOrder[name] = idx
    btn.MouseButton1Click:Connect(function() showTab(idx) end)
    return wrapTab(btn, page)
end
function Window:SelectTab(idx)
    if type(idx) == "string" then idx = _tabOrder[idx] end
    if idx and _tabs[idx] then showTab(idx) end
end
function Window:Destroy()
    SafeCall(function() if ScreenGui then ScreenGui:Destroy() end end)
end

-- Tampilkan tab pertama
if _tabs[1] then showTab(1) end

-- ---------- Fluent:Notify (custom notification) ----------
local Fluent = {}
function Fluent:Notify(data)
    pcall(function()
        ...
        notif.BackgroundColor3 = UIColors.Dark
        notif.BorderSizePixel = 0
        notif.Visible = true
        notif.ZIndex = 50
        addCorner(notif, 8)
        local s = Instance.new("UIStroke", notif); s.Color = UIColors.Accent; s.Thickness = 1; s.Transparency = 0.4
        local ttl = Instance.new("TextLabel", notif)
        ttl.Size = UDim2.new(1, -16, 0, 18)
        ttl.Position = UDim2.new(0, 8, 0, 6)
        ttl.BackgroundTransparency = 1
        ttl.Text = data.Title or "NO MERCY"
        ttl.Font = Enum.Font.GothamBold
        ttl.TextSize = 12
        ttl.TextColor3 = UIColors.Accent
        ttl.TextXAlignment = Enum.TextXAlignment.Left
        ttl.ZIndex = 51
        local body = Instance.new("TextLabel", notif)
        body.Size = UDim2.new(1, -16, 1, -26)
        body.Position = UDim2.new(0, 8, 0, 24)
        body.BackgroundTransparency = 1
        body.Text = data.Content or ""
        body.Font = Enum.Font.Gotham
        body.TextSize = 10
        body.TextColor3 = UIColors.White
        body.TextXAlignment = Enum.TextXAlignment.Left
        body.TextWrapped = true
        body.ZIndex = 51
        task.delay(data.Duration or 3, function()
            SafeCall(function()
                notif:TweenPosition(UDim2.new(1, 20, 1, -80), Enum.EasingDirection.In, Enum.EasingStyle.Quart, 0.3, true)
...
-- ---------- CreateStatus (proxy .Text supaya kode lama jalan) ----------
local function CreateStatus(tab, title, initial)
    local paragraph
    pcall(function()
        if tab and tab._page then
            paragraph = AddParagraph(tab._page, title, initial or "-")
        end
    end)
    local proxy = {}
    return setmetatable(proxy, {
        __index = function(_, key) if key == "Text" then return rawget(proxy, "_text") end return nil end,
        __newindex = function(_, key, value)
            if key == "Text" then
                rawset(proxy, "_text", value)
                pcall(function() if paragraph then paragraph:SetDesc(tostring(value)) end end)
            else rawset(proxy, key, value) end
        end,
    })
end

-- ---------- Theme listener: update UIColors saat SetTheme dipanggil ----------
local function applyUITheme(theme)
    if not theme then return end
    pcall(function()
        UIColors.Bg     = theme.Background or UIColors.Bg
        UIColors.Dark   = theme.Surface or UIColors.Dark
        UIColors.Panel  = theme.SurfaceAlt or UIColors.Panel
        UIColors.Accent = theme.Accent or UIColors.Accent
        UIColors.Border = theme.Border or UIColors.Border
        UIColors.White  = theme.Text or UIColors.White
        UIColors.TextDim = theme.TextDim or UIColors.TextDim
        MainWindow.BackgroundColor3 = UIColors.Bg
        Header.BackgroundColor3 = UIColors.Dark
        Sidebar.BackgroundColor3 = UIColors.Dark
        Container.BackgroundColor3 = UIColors.Panel
...
-- ---------- stub SaveManager / InterfaceManager (tidak ada loadstring) ----------
...
-- Daftarkan UI custom agar ikut berubah warna saat tema diganti.
AddThemeListener(applyUITheme)
-- Terapkan tema default ke UI custom sekarang.
applyUITheme(Theme or Themes["Matrix Green"])
