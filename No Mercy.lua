...
local function __ZiaanHub_Init_Main__()
    ...
    if Window then
        ...
        -- Survivor Tab -> General Section
        local GeneralTab = SurvivorTabbox1.General
        local genSection = GeneralTab:AddSection({
            Position = "Center", Name = "General Survivor", Icon = "solar:shield-bold", Box = true, BoxBorder = true, Opened = false,
        })
        genSection:AddToggle({ Default = false, Name = "Auto Skillcheck", Flag = "Auto Skillcheck", Callback = function(v) VD.AutoSkillcheck = v end })
        genSection:AddDropdown({
            Name = "Skillcheck Mode",
            Values = { "Normal", "Perfect", "Instant" },
            Default = "Normal",
            Callback = function(v) VD.AutoSkillcheckMode = type(v) == "table" and v[1] or v end
        })

        -- Survivor Tab -> Parry Section (Auto Parry & Radius)
        local ParryTab = SurvivorTabbox2.Parry
        local parrySection = ParryTab:AddSection({
            Position = "Center", Name = "Auto Parry", Icon = "solar:sword-bold", Box = true, BoxBorder = true, Opened = false,
        })
        parrySection:AddToggle({
            Name = "Enable Auto Parry", Flag = "Auto Parry", Default = false,
            Callback = function(v) VD.SURV_AutoParry = v; VD_Notify("Auto Parry", v and "Enabled" or "Disabled", 2) end
        })
        parrySection:AddSlider({
            Name = "Parry Range", Flag = "Parry Range", Min = 2, Max = 20, Default = 12, Increment = 0.5,
            Callback = function(v) VD.SURV_ParryRange = v end
        })

        -- Survivor Tab -> Gen Boost Section (Progress Bypass & Draggable Button)
        ...
        genBoostSection:AddToggle({
            Name = "Gen Boost (Progress Bypass)", Flag = "Gen_Boost", Default = false,
            Callback = function(v) VD.SURV_GenBoost = v; VD_Notify("Gen Boost", v and "Enabled" or "Disabled", 2) end
        })
        genBoostSection:AddToggle({
            Name = "Draggable Gen Bypass Button", Flag = "Draggable_Gen_Bypass", Default = false,
            Callback = function(v) VD.SURV_DraggableGenBypass = v end
        })

        -- Survivor Tab -> Movement Section (Auto Vault & Auto Pallet Slide)
        ...
        VD_Notify("ZiaanHub X", "Full Script Successfully Loaded!", 3)
    end
end
