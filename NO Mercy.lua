...
-- ============================================================
--  GLOBAL CONFIG (VD) & ZIAN FEATURE IMPLEMENTATION
-- ============================================================
...
-- ============================================================
--  ZIAN FEATURE IMPLEMENTATION (copied & adapted)
-- ============================================================
...
-- (B) HEALING (Instant Heal & Auto Heal All)
...
-- (E) VISUAL HIGHLIGHT ESP (from Zian)
do
...
VeilDraw = {
    ...
    Tracer    = Drawing and Drawing.new("Circle") or nil,
}
...
-- (L) AUTO DROP PALLET, AUTO VAULT, AUTO PALLET SLIDE
local _usedPallets = {}
...
-- Banner injection (same as original)
task.spawn(function()
    task.wait(0.3)
    local main = FindMainWindow()
...
local tpPlayerDropdown = TeleSec:AddDropdown({
...
KillerCustSec:AddDropdown({
    Name = "Custom Masked",
    Default = "Richard",
    Options = { "Richard", "Tony", "Brandon", "Jake", "Richter", "Graham", "Alex" },
    Callback = function(v) VD.KILLER_CustomMasked = v end
})
KillerCustSec:AddButton({
...
-- ============================================================
--  PLAYER TAB (Survivor Misc + Player features)
-- ============================================================
...
PlayerSurvSec:AddToggle({ Name = "First Person Camera (Survivor)", Default = false, Callback = function(v)
...
VisualESPSec:AddToggle({ Name = "ESP Skeleton (PC Only!)", Default = false, Callback = function(v)
    if VD.ESP_LowPerformance and v then return end
    VD.ESP_Skeleton = v
end })
VisualESPSec:AddToggle({ Name = "ESP Velocity Arrows (PC Only!)", Default = false, Callback = function(v)
    if VD.ESP_LowPerformance and v then return end
    VD.ESP_Velocity = v
end })
VisualESPSec:AddToggle({ Name = "ESP Offscreen Arrows (PC Only!)", Default = false, Callback = function(v)
    if VD.ESP_LowPerformance and v then return end
    VD.ESP_Offscreen = v
end })
...
SettingsSec:AddButton({
    ...
    Callback = function()
        ...
        if setclipboard then
            setclipboard(data)
            OrionLib:MakeNotification({ Name = "NO MERCY", Content = "Settings exported to clipboard!", Image = ICON.Logo, Time = 3 })
        end
    end
})
SettingsSec:AddButton({
    Name = "Import Settings (Paste from Clipboard)",
    Callback = function()
