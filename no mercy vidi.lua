--[[
=========================================================================
    NO MERCY HUB V3.5 - FINAL ULTIMATE (INVISIBLE FIXED)
    - Invisible mode SEMPURNA (player lain ga liat, kamu liat jelas)
...
local Config = {
    ...
    Invisible     = false,
    Wallhack      = false,
    LocalVisibility = 0,        -- [FIX] 0 = jelas, 0.3 = tembus, 1 = ikut ilang
}
...
local function applyInvisible(state)
    ...
    -- Loop SEMUA bagian tubuh + aksesoris + tool
    for _, part in pairs(char:GetDescendants()) do
        if part:IsA("BasePart") then
            -- Set ke 1 agar SERVER & player lain melihatmu hilang TOTAL
            part.Transparency = state and 1 or 0
        end
    end

    -- Kunci tampilan di layar kamu sendiri (TIDAK tersinkronisasi ke server)
    if state then
        humanoid.LocalTransparencyModifier = Config.LocalVisibility or 0
    else
...
print("✅ NO MERCY HUB ULTIMATE (INVISIBLE FIXED) Loaded!")
